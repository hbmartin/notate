...
import com.alexdremov.notate.feature.canvas.databinding.ActivityMainBinding
...
import com.alexdremov.notate.navigation.readCanvasDestination
...
class CanvasActivity : AppCompatActivity() {
    ...
    // Repositories are retained for document operations; session management is delegated to ViewModel.
    ...
    private lateinit var syncManager: SyncService
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val destination = intent.readCanvasDestination()
        currentCanvasPath = destination?.path
        destination?.focusBounds?.let { bounds ->
            pendingSearchBounds = RectF(bounds.left, bounds.top, bounds.right, bounds.bottom)
        }
        ...
        binding.canvasView.onBrowseFiles = { callback ->
            ...
            lifecycleScope.launch {
                ...
                val intent =
                    notateNavigator().notePickerIntent(
                        this@CanvasActivity,
                        NotePickerDestination(projectId, currentUuid),
                    )
    ...
    override fun onPause() {
        ...
        syncManager.setCanvasActive(false)
