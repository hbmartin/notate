...
class OnyxCanvasView
    ...
    constructor(
        ...
        private val textRecognition by lazy { com.alexdremov.notate.ocr.TextRecognitionFeature.get(context) }

        private suspend fun ensureOcrModelsInstalled(): Boolean {
            val manager = textRecognition.modelManager
            ...
            return try {
                ...
                if (com.alexdremov.notate.data.PreferencesManager.isBackgroundOcrIndexingEnabled(context)) {
                    textRecognition.scheduleBackfill(replace = true)
                }
                true
